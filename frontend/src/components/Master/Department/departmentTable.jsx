...
const DepartmentTableDiv = ({mode,remark,data,handleData,value}) => {
    ...
    const handleDelete=async(id)=>{    
        const data=await axios.delete(`http://localhost:8080/masterDepartment/delete/${id}`)
        console.log(data)
        handleData()
    }
    const hover={
    ...
    const columns = [
      { name: "Department Name" ,selector: (row) => row.departmentname, sortable: true,},
      {
        ...
        selector: (row) => (
              ...
              <p>
              <Flex gap='20px'>
              <DepartmentPatchModal name={<Icon as={FaEdit}  color="#3182ce" fontSize="25px" _hover={{transform:"scale(1.1)",transition:"0.5s"}}></Icon>} bgcolor={"white"} hover={hover} id={row._id} patchdepartmentname={row.departmentname}  handleData={handleData}/>
...
export default DepartmentTableDiv
