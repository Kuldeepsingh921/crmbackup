...
import DesignationPatchModal from "./designationPatchModal"
const DegignationTableDiv = ({mode,remark,data,handleData,value}) => {
  
    const handleDelete=async(id)=>{    
        const data=await axios.delete(`http://localhost:8080/masterdesignation/delete/${id}`)
        console.log(data)
        handleData()
    }
    const hover={
    ...
    const columns = [
      ...
      { name: "Designation Name" ,selector: (row) => row.designationname, sortable: true,},
      {
        ...
        selector: (row) => (
              ...
              <p>
              <Flex gap='20px'>
              <DesignationPatchModal name={<Icon as={FaEdit}  color="#3182ce" fontSize="25px" _hover={{transform:"scale(1.1)",transition:"0.5s"}}></Icon>} bgcolor={"white"} hover={hover} id={row._id} patchdepartmentname={row.departmentname} patchdesignationname={row.designationname} headername="Edit Details"  handleData={handleData}/>
...
export default DegignationTableDiv
