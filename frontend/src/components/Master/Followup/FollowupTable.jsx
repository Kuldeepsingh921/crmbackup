...
import FollowupPatchModal from "./FollowupPatchModal"
import Export from "react-data-table-component"
const FolloupTableDiv = ({mode,remark,data,handleData,value}) => {
  
    const handleDelete=async(id)=>{    
        const data=await axios.delete(`http://localhost:8080/masterfollowup/delete/${id}`)
        console.log(data)
        handleData()
    }
    const hover={
    ...
    const columns = [
      { name: "Follow Up" ,selector: (row) => row.followup, sortable: true,},
     
      {
        ...
        selector: (row) => (
              ...
              <p>
              <Flex gap='20px'>
              <FollowupPatchModal name={<Icon as={FaEdit}  color="#3182ce" fontSize="25px" _hover={{transform:"scale(1.1)",transition:"0.5s"}}></Icon>} bgcolor={"white"} hover={hover} id={row._id} patchfollowup={row.followup} handleData={handleData}/>
...
export default FolloupTableDiv
