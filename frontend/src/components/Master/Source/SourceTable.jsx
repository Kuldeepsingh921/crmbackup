...
import SourcePatchModal from "./SourcePatchModal"
const SourceTableDiv = ({mode,remark,data,handleData,value}) => {
  
    const handleDelete=async(id)=>{    
        const data=await axios.delete(`http://localhost:8080/mastersource/delete/${id}`)
        console.log(data)
        handleData()
    }
    const hover={
    ...
    const columns = [
      ...
      
      {
        ...
        selector: (row) => (
              ...
              <p>
                <Flex gap='20px'>
              <SourcePatchModal name={<Icon as={FaEdit}  color="#3182ce" fontSize="25px" _hover={{transform:"scale(1.1)",transition:"0.5s"}}></Icon>} bgcolor={"white"} hover={hover} id={row._id} patchsourcename={row.sourcename}  handleData={handleData}/>
